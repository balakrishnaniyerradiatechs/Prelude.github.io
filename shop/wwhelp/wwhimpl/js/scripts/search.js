...
function  WWHSearch_Object()
{
  ...
  this.mInitIndex             = 0;
  this.mNonBreakingCharacters = new Object();
...
{
  // Initialize non-breaking characters
  //
  this.mNonBreakingCharacters[65392] = true;  // Halfwidth Katakana-Hiragana Prolonged Sound Mark
  this.mNonBreakingCharacters[65438] = true;  // Halfwidth Katakana Voiced Sound Mark
  this.mNonBreakingCharacters[65439] = true;  // Halfwidth Katakana Semi-Voiced Sound Mark
  this.mNonBreakingCharacters[1523] = true;   // Hebrew Punctuation Geresh
  this.mNonBreakingCharacters[1524] = true;   // Hebrew Punctuation Gershayim
  this.mNonBreakingCharacters[8217] = true;   // Right Single Quotation Mark (curly apostrophe)
  this.mNonBreakingCharacters[8231] = true;   // Hyphenation Point
}
...
function  WWHSearch_ApplyWordBreaks(ParamSearchWordsString)
{
  ...
  var  VarMaxIndex;
  var  VarIndex;
  var  VarCharacterCode;
  var  VarSingleCharacterWord;
  ...
  {
    VarCharacterCode = ParamSearchWordsString.charCodeAt(VarIndex);

    // If non-extended latin or katakana, treat as a single character word.
    //
    if ((VarCharacterCode > 0x00FF) &&  
        (VarCharacterCode < 0x30A0) ||
        (VarCharacterCode > 0x30FF))
    {
      VarSingleCharacterWord = true;
 
      // Handle exceptions
      //
      if (this.mNonBreakingCharacters[VarCharacterCode] == true)
      {
        VarSingleCharacterWord = false;
      }
    }
    else
    {
    ...
    // Insert whitespace breaks
    //
    if (VarSingleCharacterWord)
    {
...
function  WWHSearch_SetSearchWords(ParamSearchWordsString)
{
  ...
  //
  VarSearchWordsString = this.fApplyWordBreaks(ParamSearchWordsString);
  SearchWordAndPhraseList = WWHStringUtilities_ParseWordsAndPhrases(VarSearchWordsString);
